  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      particles.forEach((p) => {
        ...
        ctx.fillStyle = `rgba(30,85,92,${p.opacity})`;
        ctx.fill();
      });
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 100) {
            ...
            ctx.strokeStyle = `rgba(30,85,92,${0.08 * (1 - dist / 100)})`;
            ctx.lineWidth = 0.5;
            ctx.stroke();
          }
        }
  ...
  return (
    ...
    >
      <span className="relative flex h-2 w-2">
        <span className="absolute inline-flex h-full w-full rounded-full opacity-75 animate-ping" style={{ background: "#1E555C" }} />
        <span className="relative inline-flex h-2 w-2 rounded-full" style={{ background: "#1E555C" }} />
      </span>
...
export default function Hero() {
  return (
    <section
      ...
      style={{ background: "#F4D8CD" }}
    >
      {/* Orbs */}
      <div className="orb w-[600px] h-[600px] -top-48 -right-48"
        style={{ background: "rgba(30,85,92,0.15)", filter: "blur(100px)" }} />
      <div className="orb w-[500px] h-[500px] -bottom-32 -left-32"
        style={{ background: "rgba(30,85,92,0.12)", filter: "blur(100px)", animationDelay: "3s" }} />
